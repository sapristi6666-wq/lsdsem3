const WPSD_Moderation = (function() {
    ...
    // Subtabs
    document.querySelectorAll('.wpsd-moderation-subtab').forEach(function(btn) {
        btn.addEventListener('click', function() {
            document.querySelectorAll('.wpsd-moderation-subtab').forEach(function(b) { b.classList.remove('is-active'); });
            this.classList.add('is-active');
            document.querySelectorAll('.wpsd-moderation-content').forEach(function(c) { c.style.display = 'none'; });
            var sub = this.getAttribute('data-subtab');
            if (sub === 'pending') { document.getElementById('wpsd-moderation-pending').style.display = ''; loadPending(); }
            if (sub === 'roles') { document.getElementById('wpsd-moderation-roles').style.display = ''; loadRoles(); }
        });
    });
