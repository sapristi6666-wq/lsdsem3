...
class WPSD_REST_Activities {
    ...
    public function list() {
        ...
        $q = new WP_Query(['post_type' => 'wpsd_activity', 'post_status' => ['publish'], 'posts_per_page' => 50, 'meta_key' => 'owner_user_id', 'meta_value' => $uid]);
        $items = array_map(function($p){ return ['id' => $p->ID, 'title' => $p->post_title, 'description' => $p->post_content, 'address_line1' => get_post_meta($p->ID, 'address_line1', true), 'address_line2' => get_post_meta($p->ID, 'address_line2', true), 'postal_code' => get_post_meta($p->ID, 'postal_code', true), 'city' => get_post_meta($p->ID, 'city', true), 'country' => get_post_meta($p->ID, 'country', true), 'lat' => get_post_meta($p->ID, 'lat', true), 'lng' => get_post_meta($p->ID, 'lng', true), 'photo_id' => (int)get_post_thumbnail_id($p->ID), 'photo_url' => get_the_post_thumbnail_url($p->ID, 'medium') ?: '', 'status' => $p->post_status, 'status_label' => 'Validé', 'has_accommodation' => (int)get_post_meta($p->ID, 'has_accommodation', true), 'acc_capacity' => (int)get_post_meta($p->ID, 'acc_capacity', true)]; }, $q->posts);
    ...
    public function create(WP_REST_Request $req) {
        ...
        $post_id = wp_insert_post(['post_type' => 'wpsd_activity', 'post_status' => 'publish', 'post_title' => sanitize_text_field($p['title'] ?? ''), 'post_content' => wp_kses_post($p['description'] ?? '')], true);
    ...
    public function update(WP_REST_Request $req) {
        ...
        if (isset($p['has_accommodation'])) update_post_meta($id, 'has_accommodation', !empty($p['has_accommodation']) ? 1 : 0);
        if (isset($p['acc_capacity'])) update_post_meta($id, 'acc_capacity', (int)$p['acc_capacity']);
