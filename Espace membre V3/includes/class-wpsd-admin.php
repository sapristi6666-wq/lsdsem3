...
class WPSD_Admin {
  ...
  private $template_keys = [
        'admin_new_registration' => ['Admin - Nouvelle inscription', '{{prenom}} {{nom}} {{email}} {{role}} {{plan}} {{admin_url}}'],
    'user_pending'           => ['Utilisateur - Inscription reçue', '{{prenom}} {{nom}} {{email}} {{role}} {{plan}}'],
    'user_approved'          => ['Utilisateur - Compte approuvé', '{{display_name}} {{email}} {{role}} {{reset_url}}'],
    'user_rejected'          => ['Utilisateur - Compte refusé', '{{prenom}} {{nom}} {{email}}'],
    ...
    add_action('rest_api_init', [$this, 'register_email_test_route']);
    add_action('wp_ajax_wpsd_save_test_email', [$this, 'save_test_email']);
  }
  ...
  public function email_templates_page() {
    ...
    <div class="wrap"><h1>Templates d'emails</h1>
      <div style="background:#f0fdf4;border:1px solid #bbf7d0;border-radius:8px;padding:12px 16px;margin-bottom:16px;display:flex;align-items:center;gap:12px;flex-wrap:wrap;">
        <strong>📧 Email de test :</strong>
        <input type="email" id="wpsd_test_email" value="<?=esc_attr(get_option('wpsd_test_email',''))?>" placeholder="test@exemple.fr" style="min-width:220px;">
        <button class="button button-secondary" id="wpsd_save_test_email">Enregistrer</button>
        <span id="wpsd_test_email_msg" style="font-size:12px;color:#666;"></span>
      </div>
...
<script>
document.getElementById('wpsd_save_test_email')?.addEventListener('click', function() {
  var val = document.getElementById('wpsd_test_email').value.trim();
  if (!val) { document.getElementById('wpsd_test_email_msg').textContent = 'Veuillez entrer un email.'; return; }
  var form = new FormData(); form.append('action', 'wpsd_save_test_email'); form.append('email', val); form.append('_wpnonce', '<?=wp_create_nonce('wpsd_save_test_email')?>');
  fetch(ajaxurl, { method: 'POST', body: form }).then(function(r){ return r.text(); }).then(function(){ document.getElementById('wpsd_test_email_msg').textContent = 'Enregistre.'; setTimeout(function(){ document.getElementById('wpsd_test_email_msg').textContent = ''; }, 2000); });
});
document.querySelectorAll('[data-test-template]').forEach(function(btn) {
  btn.addEventListener('click', function() {
    var template = this.getAttribute('data-test-template');
    var email = document.getElementById('wpsd_test_email').value.trim();
    if (!email) { alert('Veuillez d\'abord definir une adresse email de test.'); return; }
    this.disabled = true; this.textContent = 'Envoi...';
    fetch('<?=rest_url('wpsd/v2/admin/emails/test')?>', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json', 'X-WP-Nonce': '<?=wp_create_nonce('wp_rest')?>' },
      body: JSON.stringify({ template_id: template, test_email: email })
    }).then(function(r){ return r.json(); }).then(function(d){
      if (d.ok) alert('Email de test envoye a ' + email);
      else alert('Erreur : ' + (d.error || 'inconnue'));
      btn.disabled = false; btn.textContent = 'Tester';
    }).catch(function(){
      alert('Erreur réseau'); btn.disabled = false; btn.textContent = 'Tester';
    });
  });
});
</script>
    ...
    <form method="post" style="margin-top:20px;"><?php wp_nonce_field('wpsd_email_templates'); ?>
        ...
        <div id="tab-<?=$k?>" style="<?=$active_tab===$k?'':'display:none;'?>">
          ...
          <table class="form-table">
            ...
            <tr><th><label>Contenu (HTML)</label></th><td><textarea name="body_<?=$k?>" rows="12" class="large-text"><?=esc_textarea($this->get_template($k,'body'))?></textarea><p class="description">Variables : <?=$info[1]?><br><button type="button" class="button" data-test-template="<?=$k?>" style="margin-left:0;margin-top:4px;">Tester cet email</button></p></td></tr>
          </table>
        </div>
      <?php endforeach; ?>
            ...
            <tr>
              ...
              <td>
                ...
                <p class="description">
                  ...
                  <br><button type="button" class="button" data-test-template="<?=$event?>" style="margin-left:0;margin-top:4px;">Tester cet email</button>
                </p>
              </td>
            </tr>
          </table>
  ...
  // ==================== EMAIL TEST (REST + AJAX) ====================

  public function register_email_test_route() {
    register_rest_route('wpsd/v2', '/admin/emails/test', [
      'methods' => 'POST',
      'callback' => [$this, 'handle_email_test'],
      'permission_callback' => fn() => current_user_can('manage_options'),
    ]);
  }

  public function handle_email_test($request) {
    $template_id = $request->get_param('template_id');
    $test_email  = sanitize_email($request->get_param('test_email'));

    if (!$template_id || !$test_email) {
      return ['ok' => false, 'error' => 'Paramètres manquants (template_id, test_email)'];
    }

    $fakedata = [
      'admin_new_registration' => [
        'prenom' => 'Jean', 'nom' => 'Test', 'email' => 'jean.test@example.com',
        'role' => 'Itinérant', 'plan' => 'Individuel',
        'admin_url' => admin_url('admin.php?page=wpsd-pending-registrations'),
      ],
      'user_pending' => [
        'prenom' => 'Marie', 'nom' => 'Exemple', 'email' => 'marie@example.com',
        'role' => 'Passeur de savoir', 'plan' => 'Famille',
      ],
      'user_approved' => [
        'display_name' => 'Marie Exemple', 'email' => 'marie@example.com',
        'role' => 'Passeur de savoir',
        'reset_url' => home_url('/wp-login.php?action=rp&key=TESTKEY&login=marie'),
      ],
      'user_rejected' => [
        'prenom' => 'Paul', 'nom' => 'Refus', 'email' => 'paul@example.com',
      ],
      'subscription_active' => [
        'display_name' => 'Marie Exemple', 'plan_label' => 'Famille (70€/an)',
        'status' => 'actif', 'account_url' => home_url('/mon-compte/'),
      ],
      'renewal_reminder' => [
        'display_name' => 'Marie Exemple', 'date_fr' => '15 juin 2026',
        'account_url' => home_url('/mon-compte/'),
      ],
      'admin_created_account' => [
        'first_name' => 'Luc', 'email' => 'luc@example.com',
        'reset_url' => home_url('/wp-login.php?action=rp&key=TESTKEY&login=luc'),
      ],
    ];

    // Réservation templates fallback
    $reservation_data = [
      'display_name' => 'Jean Test',
      'provider_name' => 'Marie Passeur',
      'object_title' => 'Initiation à la vannerie',
      'date_start' => '22/05/2026',
      'date_end' => '23/05/2026',
      'days' => '2',
    ];

    // 1) Try registration templates
    if (isset($this->template_keys[$template_id])) {
      $subject = $this->get_template($template_id, 'subject');
      $body    = $this->get_template($template_id, 'body');
      $data    = $fakedata[$template_id] ?? [];
    }
    // 2) Try reservation templates
    elseif (in_array($template_id, array_keys(WPSD_Data::EMAIL_TEMPLATES), true)) {
      $subject = WPSD_Data::get_email_template($template_id, 'subject');
      $body    = WPSD_Data::get_email_template($template_id, 'body');
      $data    = $reservation_data;
    }
    else {
      return ['ok' => false, 'error' => 'Template inconnu : ' . $template_id];
    }
    ...
    // Add banner to body
    $banner = '<p style="color:#d00;font-weight:bold;">Ceci est un email de test. Destinataire original simulé : ' . esc_html($test_email) . '</p>';
    $body = str_replace('<div style="padding:24px;color:#1a2e2a;font-size:15px;line-height:1.6;">', '<div style="padding:24px;color:#1a2e2a;font-size:15px;line-height:1.6;">' . $banner, $body);
    ...
    return ['ok' => false, 'error' => 'Échec de l\'envoi via wp_mail()'];
  }

  public function save_test_email() {
    check_ajax_referer('wpsd_save_test_email');
    $email = sanitize_email($_POST['email'] ?? '');
    if ($email) {
      update_option('wpsd_test_email', $email);
    }
    wp_die('ok');
  }
