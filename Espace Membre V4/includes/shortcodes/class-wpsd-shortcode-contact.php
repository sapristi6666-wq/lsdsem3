...
class WPSD_Shortcode_Contact {
    ...
    public function render() {
        ...
        $subjects_raw = wp_unslash(get_option('wpsd_contact_subjects', "Demande d'information\nQuestion sur l'adhésion\nProblème technique\nAutre"));
    ...
    public function ajax_submit() {
        ...
        $email_subject = wp_unslash(get_option('wpsd_contact_email_subject', 'Contact site')) . ' : ' . $subject;
        $template = wp_unslash(get_option('wpsd_contact_email_template', $this->default_template()));
        
        $html = str_replace(
            ['{{lastname}}','{{firstname}}','{{email}}','{{phone}}','{{subject}}','{{message}}','{{date}}','{{site_name}}'],
            [esc_html($lastname),esc_html($firstname),esc_html($email),esc_html($phone?:'—'),esc_html($subject),nl2br(esc_html($message)),date_i18n('d/m/Y à H:i'),esc_html(get_bloginfo('name'))],
            $template
        );
        ...
        if (wp_mail($recipient, $email_subject, $html, $headers)) 
            wp_send_json_success(['message' => wp_unslash(get_option('wpsd_contact_success_message', 'Votre message a été envoyé avec succès !'))]);
        else 
    ...
    public function settings_init() { 
        foreach(['wpsd_contact_subjects','wpsd_contact_recipient','wpsd_contact_email_subject','wpsd_contact_email_template','wpsd_contact_success_message'] as $o) 
            register_setting('wpsd_contact_settings',$o); 
    }
    ...
    public function settings_page() {
        ...
        <table class="form-table">
            ...
            <tr><th>Préfixe du sujet</th><td><input type="text" name="wpsd_contact_email_subject" value="<?= esc_attr(wp_unslash(get_option('wpsd_contact_email_subject','Contact site'))) ?>" class="regular-text"></td></tr>
            <tr><th>Message de succès</th><td><input type="text" name="wpsd_contact_success_message" value="<?= esc_attr(wp_unslash(get_option('wpsd_contact_success_message','Votre message a été envoyé avec succès !'))) ?>" class="large-text"></td></tr>
            <tr><th>Liste des sujets</th><td><textarea name="wpsd_contact_subjects" rows="6" class="large-text"><?= esc_textarea(wp_unslash(get_option('wpsd_contact_subjects',"Demande d'information\nQuestion sur l'adhésion\nProblème technique\nAutre"))) ?></textarea></td></tr>
            <tr><th>Template de l'email</th><td><textarea name="wpsd_contact_email_template" rows="15" class="large-text"><?= esc_textarea(wp_unslash(get_option('wpsd_contact_email_template',$this->default_template()))) ?></textarea></td></tr>
